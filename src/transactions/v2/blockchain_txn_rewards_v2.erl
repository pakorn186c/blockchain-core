...
-record(rewards_shares, {
          ...
          shares = 0 :: non_neg_integer() %% shares of a reward type
}).

-type rewards_shares() :: #rewards_shares{}.

-record(rewards_meta, {
          hnt_amount = 0 :: non_neg_integer(), %% actual HNT value (rolling accumulator)
...
%% too frequently
-type rewards_metadata() :: #{ libp2p_crypto:pubkey_bin() => rewards_meta(),
                               shares_acc => #{ poc_challengers => non_neg_integer(),
...
finalize_reward_calculations(#{shares_acc := Shares} = RewardsMD, Ledger, Vars) ->
    ...
    Vars0 = Vars#{dc_remainder => DCRemainder,
                  dc_reward => DCReward},
                            ...
                            case blockchain_ledger_state_channel_v2:is_v2(SC) of
                                true ->
                                    ...
                                    lists:foldl(fun(Summary, A) ->
                                                        ...
                                                        case blockchain_ledger_v1:find_gateway_owner(Key) of
                                                            {ok, GWOwner} ->
...
-spec update_hnt(RewardsMeta :: rewards_meta(),
                 Amount :: non_neg_integer()) -> rewards_meta().
...
-spec update_hnt(RewardsMD :: rewards_metadata(),
                 ...
                 Amount :: non_neg_integer() ) -> rewards_metadata().
...
-spec update_seen(SCID :: blockchain_state_channel_v1:id(),
                  RewardsMD :: rewards_meta() ) -> NewRewardsMD :: rewards_meta().
...
-spec new_rewards_shares( Type :: reward_types(),
                          Amount :: non_neg_integer() ) -> RewardsShares :: rewards_shares().
...
get_owner_address(gateway, Addr, Ledger) ->
    blockchain_ledger_v1:get_gateway_owner(Addr, Ledger).

-spec calculate_dc_remainder(rewards_metadata(), rewards_vars()) -> non_neg_integer().
...
-spec normalize_dc_rewards( RewardsMD :: rewards_metadata(),
                            DCReward :: non_neg_integer() ) -> rewards_metadata()}.
normalize_dc_rewards(RewardsMD, DCReward)
     maps:fold(
       fun(Key, NumDCs, Acc) ->
